...
if __name__ == '__main__':
    try:
        ...
        with open('decrypted_chrome_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
...
def get_secret_key():
    try:
        #(1) Get secretkey from brave local state
        ...
        print("%s"%str(e))
        print("[ERR] brave secretkey cannot be found")
        return None
...
def decrypt_password(ciphertext, secret_key):
    try:
        ...
        print("%s"%str(e))
        print("[ERR] Unable to decrypt, brave version <80 not supported. Please check.")
        return ""
    
def get_db_connection(BRAVE_PATH_login_db):
    try:
        print(BRAVE_PATH_login_db)
        shutil.copy2(BRAVE_PATH_login_db, "Loginvault.db") 
        return sqlite3.connect("Loginvault.db")
    except Exception as e:
        print("%s"%str(e))
        print("[ERR] brave database cannot be found")
        return None
        
if __name__ == '__main__':
    try:
        #Create Dataframe to store passwords
        with open('decrypted_brave_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
            csv_writer = csv.writer(decrypt_password_file, delimiter=',')
            csv_writer.writerow(["index","url","username","password"])
            #(1) Get secret key
            secret_key = get_secret_key()
            #Search user profile or default folder (this is where the encrypted login password is stored)
            folders = [element for element in os.listdir(BRAVE_PATH) if re.search("^Profile*|^Default$",element)!=None]
            for folder in folders:
            	#(2) Get ciphertext from sqlite database
                BRAVE_PATH_login_db = os.path.normpath(r"%s\%s\Login Data"%(BRAVE_PATH,folder))
                conn = get_db_connection(BRAVE_PATH_login_db)
                if(secret_key and conn):
                    cursor = conn.cursor()
                    cursor.execute("SELECT action_url, username_value, password_value FROM logins")
                    for index,login in enumerate(cursor.fetchall()):
...
def decrypt_password(ciphertext, secret_key):
    try:
        ...
        print("%s"%str(e))
        print("[ERR] Unable to decrypt, Opera version <80 not supported. Please check.")
        return ""
    
def get_db_connection(OPERA_PATH_login_db):
    try:
        print(OPERA_PATH_login_db)
        shutil.copy2(OPERA_PATH_login_db, "Loginvault.db") 
        return sqlite3.connect("Loginvault.db")
    except Exception as e:
        print("%s"%str(e))
        print("[ERR] Opera database cannot be found")
        return None
        
if __name__ == '__main__':
    try:
        #Create Dataframe to store passwords
        with open('decrypted_opera_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
            csv_writer = csv.writer(decrypt_password_file, delimiter=',')
            csv_writer.writerow(["index","url","username","password"])
            #(1) Get secret key
            secret_key = get_secret_key()
            ...
            for folder in folders:
            	#(2) Get ciphertext from sqlite database
                OPERA_PATH_login_db = os.path.normpath(r"%s\%s\Login Data"%(OPERA_PATH,folder))
...
if __name__ == '__main__':
    try:
        #Create Dataframe to store passwords
        with open('decrypted_edge_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
            csv_writer = csv.writer(decrypt_password_file, delimiter=',')
            csv_writer.writerow(["index","url","username","password"])
            #(1) Get secret key
            secret_key = get_secret_key()
